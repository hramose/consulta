...
class PatientController extends ApiController
{
    ...
    public function getHistory($id)
    {
       $patient = $this->patientRepo->findById($id);
       $history = $patient->history;
       $appointments = $patient->appointments()->with('user','diagnostics','diseaseNotes','labexams','treatments')->where('status', 1)->orderBy('start','DESC')->limit(3)->get();//$patient->appointments->load('user','diagnostics');
